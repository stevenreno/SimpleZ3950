...
//		}
	...
	public function Search() {
		...
		if(!function_exists("yaz_connect")) {
			...
			yaz_syntax($vo_yaz_ressource, $va_server["marc"]);
			...
			$this->view->setVar("mapping", $va_server["mapping"]);
	...
	public function Import() {
		...
		$vs_mapping = $this->request->getParameter('mapping', pString);
		//var_dump($vn_results);
		$files=[];
		$commands=[];
		$outputs=[];
		for($i=0;$i<$vn_results;$i++) {
			...
			if($filepath != "") {
				$files[] = $filepath;
				$command= "cd ".__CA_BASE_DIR__."/support && ./bin/caUtils import-data -s ".$filepath." -m ".$vs_mapping." -f marc -l . -d DEBUG";
				//print $command;
